...
namespace CompactView
{
	public class ColorComboBox
	{
        ...
        public static void SetectColor(ComboBox comboBox, Color color)
        {
            ...
            if (i < 0) i = argbColors.IndexOf(color.ToArgb());
            comboBox.ForeColor = i < 0 ? color : Color.FromName(colorNames[i]);
            comboBox.SelectedIndex = i;
            if (i < 0) comboBox.ForeColor = color;  // if set SelectedIndex < 0 the calling to event SelectedIndexChanged will set the color as transparent
        }
